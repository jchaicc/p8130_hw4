...
```{r}
heart_new_data[is.na(heart_new_data) | heart_new_data=="-Inf"] = NA
multi_reg <- lm(log_totalcost~e_rvisits+comp_bin,data = heart_new_data)
...
```{r}
lm(log_totalcost~factor(comp_bin)+e_rvisits+factor(comp_bin)*e_rvisits,data = heart_new_data) %>%
 summary()

lm(log_totalcost~factor(comp_bin)*e_rvisits,data = heart_new_data) %>% 
  summary()
